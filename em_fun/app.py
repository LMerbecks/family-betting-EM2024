...
def save_votes(username, votes):
    ...
    for vote_id, vote in votes.items():
        ...
        c.execute("INSERT INTO votes (id, vote_id, username, date) VALUES (?, ?, ?, ?)", (unique_id, vote_id, username, vote['date']))
    conn.commit()
    conn.close()
...
if not st.session_state.logged_in:
    ...
    if st.button('Submit'):
        ...
        votes = {}
        for group, positions in group_positions.items():
            votes[f'{group}_1st_place'] = {'username': st.session_state.username, 'date': str(datetime.now())}
            votes[f'{group}_2nd_place'] = {'username': st.session_state.username, 'date': str(datetime.now())}
        votes['champion'] = {'username': st.session_state.username, 'date': str(datetime.now())}
        votes['vice_champion'] = {'username': st.session_state.username, 'date': str(datetime.now())}
        votes['third_place'] = {'username': st.session_state.username, 'date': str(datetime.now())}
        votes['fourth_place'] = {'username': st.session_state.username, 'date': str(datetime.now())}
